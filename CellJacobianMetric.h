...
VTKM_EXEC inline OutType ComputeJacobianOfQuad(const VecType& edgeCalculations, const VecType& axes)
{
...
template<typename OutType, typename VecType>
VTKM_EXEC inline OutType ComputeJacobianOfHex(const VecType& matrices)
{
  ...
  minDeterminant = (OutType)vtkm::Dot(matrices[numMatrices-1][0], vtkm::Cross(matrices[numMatrices-1][1], matrices[numMatrices-1][2]));
  minDeterminant /= 64.0;
  for (vtkm::IdComponent i = 0; i < numMatrices-1; i++)
  {
    currDeterminant = (OutType)vtkm::Dot(matrices[i][0], vtkm::Cross(matrices[i][1], matrices[i][2]));
...
VTKM_EXEC OutType ComputeJacobian(const vtkm::IdComponent& numPts,
                                 ...
                                 const vtkm::exec::FunctorBase& worklet)
{
  ...
  {
    case 4:
            return ComputeJacobian<OutType>(numPts, pts, vtkm::CellShapeTagQuad(), worklet);
    default:
            break;
  }
  return OutType(-1.0);
...
template <typename OutType, typename PointCoordVecType>
VTKM_EXEC OutType ComputeJacobian(const vtkm::IdComponent&,
                              ...
                              const vtkm::exec::FunctorBase& worklet)
{
  ...
  const Edge QuadAxes[2] = {QuadEdges[0] - (pts[2] - pts[3]),
                            QuadEdges[1] - (pts[3] - pts[0])
                           };
  ...
  return vtkm::exec::cellmetrics::ComputeJacobianOfQuad<OutType>(vtkm::make_VecC(QuadEdgesToUse, 4), vtkm::make_VecC(QuadAxes, 2));
}
...
{ 
  ...
  return vtkm::exec::cellmetrics::ComputeJacobianOfHex<OutType>(vtkm::make_VecC(hexMatrices, 12));
}
                                 ...
                                 const vtkm::exec::FunctorBase& worklet)
{
  ...
  //the 3 edges we need
  using Edge = typename PointCoordVecType::ComponentType;
  const Edge EdgesNeeded[3] = {pts[1] - pts[0],
                                pts[0] - pts[2],
                                pts[3] - pts[0]};
